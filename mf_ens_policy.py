...
while budget_left > 0:
    ...
    for point in test_idx:
        ...
        for Y_x in [0, 1]:
            ...
            budget_left_fake1 = budget_left - c1
            budget_left_fake2 = budget_left - c2
            max_fake1_t1 = int(np.floor(budget_left_fake1/c1))
            max_fake1_t2 = int(np.floor(budget_left_fake1/c2))
            max_fake2_t1 = int(np.floor(budget_left_fake2/c1))
            max_fake2_t2 = int(np.floor(budget_left_fake2/c2))

            temp_app = -10000
            if budget_left_fake1 < c1:
                temp_app = 0
            for t1 in range(max_fake1_t1):
                t2 = int(budget_left_fake1 - t1 * c1)
                if t2 < 0:
                    t2 = 0
            ...
            temp_app = -10000
            if budget_left_fake2 < c1:
                temp_app = 0
            for t1 in range(max_fake2_t1):
                t2 = int(budget_left_fake2 - t1 * c1)
                if t2 < 0:
                    t2 = 0
                next_batch = next_probs.nlargest(min(t1+t2, len(test_fake_idx)))
                temp_sum = p2*next_batch[0:t2].sum() + p1*next_batch[t2:t1+t2].sum()
